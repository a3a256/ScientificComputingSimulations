...
public class ParallelAirfoil extends Thread {
    ...
    static double [] [] vel, rho;

    static double [] [] [] fin, fout;
    ...
    public static void main(String args []) throws Exception {
        ...
        noslip = new int [Q] ;  // index in c of negative velocity state
        for(int i = 0 ; i < Q ; i++) {
            int [] cEl = c [i] ;
            for(int j = 0 ; j < Q ; j++) {
                int [] cElj = c [j] ;
        ...
        fout = new double [NX] [NY] [Q] ;
        rho = new double [NX] [NY] ;

        ParallelAirfoil[] threads = new ParallelAirfoil[P];

        long startTime = System.currentTimeMillis();

        for(int i = 0 ; i < P ; i++){
            threads[i] = new ParallelAirfoil(i);
            threads[i].start();
        }
